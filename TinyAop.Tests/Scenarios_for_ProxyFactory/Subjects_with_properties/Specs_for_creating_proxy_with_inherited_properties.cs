...
{
    [TestFixture]
    public class Specs_for_creating_proxy_with_inherited_properties : BDD<Specs_for_creating_proxy_with_inherited_properties>
    {
        private SubjectWithOneInheritedProperty _subject;
        private ISubjectWithOneInheritedProperty _proxy;
        ...
        {
            _proxy = new ProxyFactory().Create<ISubjectWithOneInheritedProperty>(_subject);
        }

        private void We_have_a_subject_with_one_property_to_intercept()
        {
            _subject = new SubjectWithOneInheritedProperty();
        }
    }
}